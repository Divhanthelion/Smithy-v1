...
- Filesystem tools (`read`, `write`, `edit`, `ls`, `glob`, `grep`) cover the Project and the scratch directory. Scratch is for archives and other files that should not live in the Project. `/tmp` itself is not open — copy into scratch (or the Project) first. `bash` can reach elsewhere, and waits for the user.
